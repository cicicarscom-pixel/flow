...
### [17.09.2026] "Tüm Gönderiler" Ekranında Yanlış İçerik Yer Tutucusu Hatası Düzeltildi

**Sorun:** Kullanıcı ekran görüntüleriyle bildirdi: "Tüm Gönderiler" (`PostsScreen.js`) listesinde gerçek gönderi içeriği/başlığı yerine "Bu yorum gizlendi." metni görünüyordu.

**Kök neden:** `PostsScreen.js` satır 320'deki içerik önizleme fallback'i (`getPreviewText(item.content || item.title) || t(...)`), muhtemelen `PostCommentsScreen.js`'teki (satır ~423) benzer bir satırdan kopyalanırken, gizlenmiş bir YORUM için kullanılan `postCommentsScreen.hiddenComment` çeviri anahtarını ("Bu yorum gizlendi.") yanlışlıkla GÖNDERİ boş-içerik yer tutucusu olarak yeniden kullanıyordu. Bu anahtar `tr.json`/`en.json`/`de.json` içinde gerçekten mevcut olduğu için i18next'in `defaultValue: 'Açıklama yok'` parametresi hiçbir zaman devreye girmiyordu (i18next `defaultValue`'yu SADECE anahtar hiç bulunamadığında kullanır) — sonuç olarak `item.content || item.title` boş dönen her gönderide, gerçek içerik yerine hep "Bu yorum gizlendi." yazıyordu.

**Çözüm:** Yanlış `postCommentsScreen.hiddenComment` anahtarı yerine, `src/core/i18n/locales/{tr,en,de}.json`'daki mevcut `postsScreen` namespace'ine eklenen özel bir `postsScreen.noContent` anahtarı (tr: "Açıklama yok", en: "No description", de: "Keine Beschreibung") kullanılacak şekilde `PostsScreen.js` güncellendi.
...
**Kapsam kararı:** `ledger` (mali müşavir paneli) bu turun dışında bırakıldı — VKN/KDV/tevkifat/e-fatura gibi Türk vergi mevzuatına özgü kavramlar sadece arka plan mantığında değil, doğrudan arayüz metinlerinde ve AI belge-ayrıştırma şemasında birinci sınıf kavramlar olarak yer alıyor; kendi dokümantasyonunda da uluslararası genişleme sinyali yok. `flow` (mobil) zaten kısmi `react-i18next` altyapısına sahipken, `flowweb` (web panel) sıfır i18n altyapısıyla başlıyordu — bu yüzden ikisi çok farklı noktadan başlıyor.

**Mobil (flow) — bu turda tamamlanan iş:**

1. **Otomatik algılama + manuel değiştirme + hatırlama (kullanıcının asıl istediği özellik):** `src/core/i18n/index.ts` yeniden yazıldı. İlk açılışta cihazın işletim sistemi diline göre otomatik algılama zaten vardı (`expo-localization`); eksik olan kısım — kullanıcının Ayarlar'dan manuel dil seçebilmesi ve bu seçimin hatırlanması — eklendi: `setAppLanguage()` (seçimi anında uygular + `AsyncStorage`'a yazar) ve `clearAppLanguageOverride()` (cihaz diline geri döner) fonksiyonları eklendi. `src/screens/ProfilScreen.js`'e bu 3 dili (Türkçe/English/Deutsch) gösteren bir dil seçici bileşeni (`LanguageSwitcher`) eklendi.
2. **Çeviri dosyaları tamamlandı:** `en.json`'da eksik olan `sosyalMedya.bot.*` altında 19 anahtar (systemInstruction, liveTest, aiKnowledge, whatsappConnection vb.) İngilizce çevirileriyle eklendi; yanlış çevrilmiş `status` anahtarı ("Bot Status" → "Status:") düzeltildi; kullanılmayan yetim anahtar (`aiAccount`) temizlendi. `de.json` daha ciddi bir durumdaydı — sadece `randevu` ve `musteriler` modüllerini kapsıyordu, `muhasebe` ve `sosyalMedya` modülleri (uygulamanın büyük çoğunluğu) tamamen eksikti; her iki modül de sıfırdan Almanca çevrilerek eklendi. Ayrıca her 3 dile de yeni bir `common.language.*` (dil seçici metinleri) namespace'i eklendi.
3. **İlk tam ekran örneği:** `ProfilScreen.js` (Profil/Ayarlar ekranı — dil seçicinin de yaşadığı yer) baştan sona `useTranslation()`'a taşındı; yeni bir `profil.*` namespace'i 3 dile de eklendi.

**Denetimde bulunan, henüz düzeltilmeyen (sonraki dalgaya bırakılan) mobil bulgular:** `src/screens/**` altındaki 8 ekranın (AuthScreen, DashboardScreen, AiChatScreen, AiAssistantScreen, BildirimlerScreen, PostsScreen, PostCommentsScreen dahil) **hiçbiri** `useTranslation` kullanmıyor — giriş/kayıt ekranı (AuthScreen) dahil tamamı hardcoded Türkçe. Ayrıca 13 yerde `.toLocaleDateString('tr-TR', ...)` gibi doğrudan Türkçe locale hardcode edilmiş, 7+ yerde `₺` sembolü (bazen zaten locale-aware `formatCurrency()` ile birlikte, çift sembol riski) sabit yazılmış, ve `eslint-plugin-i18next` linter kuralı tam olarak bu en sorunlu dosyaları (`src/screens/**`) kapsam dışı bırakıyor. Toplam ~437 hardcoded Türkçe satır tespit edildi (62 dosyanın sadece %18'i i18n kullanıyor).

**Web (flowweb) — bu turun kapsamına henüz alınmadı, ayrı bir mimari karar gerektiriyor:** Web panelinde **hiçbir** i18n altyapısı yok (next-intl/i18next yok, tarayıcı dili algılama yok, `<html lang="en">` bile içerikle uyumsuz — her şey Türkçe). ~500-650 hardcoded Türkçe string, ~35-40 dosyaya yayılmış durumda (en yoğunları: `gelen-kutusu/page.tsx` 1105 satır, `sosyal-medya/share/page.tsx` 806 satır, `ai-asistan/page.tsx` 711 satır). En kritik mimari risk: `AICharacterPanel.tsx`'teki AI persona rol/ton isimleri (`"Kebapçı"`, `"Standart"` vb.) hem görünen etiket hem de `organization_ai_settings` tablosuna yazılan veri anahtarı olarak kullanılıyor — önce stabil bir `id`/çevrilebilir `label` ayrımı yapılmadan metinleri çevirmek, veritabanındaki kayıtlı verileri ve mobil ile paylaşılan AI mantığını bozar. Web tarafı, dashboard/panel yapısı olduğu (SEO'nun önemli olmadığı) için URL'lere `/en/`, `/de/` gibi dil öneki eklemek yerine, çerez tabanlı (URL değişmeyen) bir `next-intl` kurulumu öneriliyor — bu bir sonraki geliştirme turunda ele alınacak.
...
1. `handle_new_user()` tetikleyicisi artık her yeni kullanıcı için otomatik `organizations`/`organization_members` kaydı da açıyor (önceden sadece `profiles` açıyordu, bireysel/freelancer kullanıcılar Zernio'ya hiç bağlanamıyordu).
2. `integration.social_accounts` / `integration.zernio_profiles` şemasına `service_role` ve `authenticated` rolleri için eksik olan temel `GRANT USAGE`/`SELECT`/`INSERT` izinleri eklendi — bu olmadan bu şemaya yapılan HİÇBİR sorgu (RLS'den bağımsız) çalışmıyordu.
3. `HandleIncomingMessageUseCase.ts`'teki (AI'nin hangi Zernio hesabı üzerinden yanıt vereceğini bulan) sorgu, eski `public.social_accounts`/`profile_id` yerine güncel `integration.social_accounts`/`organization_id`'ye taşındı — bu, AI'nin Zernio üzerinden bağlanan hesaplara (Instagram/Facebook/WhatsApp) yanıt verebilmesini doğrudan etkiliyordu ve mobil tarafta da aynı fonksiyon kullanıldığı için mobil müşteri mesajlaşmasını da kapsıyor.
