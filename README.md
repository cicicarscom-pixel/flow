...
21. **Sistem Talimatı Kartına Çerçeveyi Takip Eden RGB Işık Halesi ve Sabit Yükseklikli Scrollable Giriş Kutusu (UX/UI):**
    - **Neden:** Kartın arkasında dönen büyük kare gradyan katmanı, dönüş hareketi esnasında köşelerinin (kulaklarının) dışa taşmasına ve çirkin bir dönen kare kulak efekti oluşturmasına sebep oluyordu. Kullanıcı, ışık halesinin ve gölgesinin kartın yuvarlatılmış köşelerini tam olarak takip etmesini (çerçeveyi takip eden gölge) talep etti. Ayrıca, asistan talimatı kutusuna uzun yönergeler yapıştırıldığında kutunun kontrolsüz şekilde uzaması yerine, sabit bir yükseklikte kalıp kendi içinde kaydırılabilir (scrollable) olması istendi.
    - **Çözüm:** `BotYonetimiScreen.js` dosyasında şu düzenlemeler yapıldı:
        - **Çerçeveyi Takip Eden Gölge (Halo/Glow):** Kartın arkasındaki serbest dönen kare gradyan kaldırıldı. Bunun yerine en dışa `borderRadius: 20` olan ve `shadowColor: '#00f0ff'`, `shadowRadius: 18`, `shadowOpacity: 0.85` özellikleriyle donatılmış sabit bir dış sarmal `View` yerleştirildi. Bu sayede gölge ışık halesi kartın kendi yuvarlatılmış köşelerini mükemmel bir şekilde takip edecek (çerçeveyi takip eden gölge) duruma getirildi.
        - **İç Dönen RGB Işık Çerçevesi (Border Beam):** Kartın `overflow: 'hidden'` olan kenarlık alanının (border) içerisine, dairesel olarak dönen bir `conic-gradient` simülasyonu (Animated `LinearGradient`) yerleştirildi. Bu sayede renkler ince bir ışık çizgisi halinde çerçeve boyunca akıcı ve dönen bir aura gibi hareket etmektedir.
