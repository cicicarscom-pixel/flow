...
21. **Sistem Talimatı Kartına Conic Gradient RGB Işık Çerçevesi ve Aura Gölgesi (UX/UI):**
    - **Neden:** Kartın arkasında dönen gradyan katmanı bir kare/dikdörtgen olduğu için köşeleri dönme esnasında dışa taşmaktaydı. Ayrıca linear gradyanlar tek doğrultuda değiştiğinden tam bir conic gradient (çerçeveyi saran kesintisiz renk akışı) hissi vermiyordu. Kullanıcı, Google Stitch tasarımına benzer şekilde kartın yuvarlatılmış köşelerini takip eden parlayan kesintisiz bir RGB conic çerçeve ve onun arkasında aura şeklinde dönen bir RGB gölge (conic glow shadow) talep etti. Ayrıca, dikdörtgen olan kartın arkasında doğrudan `width: 100%` olarak döndürülen görselin ovalleşerek "dönen bir kart/dikdörtgen" gibi yalpalaması sorunu vardı.
    - **Çözüm:** `BotYonetimiScreen.js` dosyasında şu düzenlemeler yapıldı:
        - **Conic Gradient Varlık (conic_glow.png):** `cyan`, `magenta`, `yellow`, `blue` ve `green` geçişlerini içeren pürüzsüz dairesel bir renk çarkı (conic gradient) imajı oluşturuldu ve `src/core/assets/images/conic_glow.png` konumuna kaydedildi.
        - **Dönen RGB Aura Gölgesi:** Kartın tam ortasına merkezlenmiş, `aspectRatio: 1` ile **mükemmel kare** olması zorlanmış ve `blurRadius` (iOS: 25, Android: 12) ile yumuşatılmış dairesel bir `Animated.Image` (conic_glow) yerleştirildi. Mükemmel kare geometrisi sayesinde, dönerken hiçbir yalpalama veya kart benzeri köşeli hareket yapmadan tamamen simetrik ve pürüzsüz dönen bir RGB aura gölgesi elde edildi.
        - **İç Dönen RGB Işık Çerçevesi (Border Beam):** Kartın `overflow: 'hidden'` olan 2.5px'lik kenarlık (padding) alanının içerisine, yine tam merkezde `aspectRatio: 1` kare şeklinde dairesel olarak dönen `conic_glow` imajı yerleştirilerek yalpalama yapmayan, çerçeve boyunca pürüzsüzce akan kesintisiz bir conic border beam oluşturuldu.
