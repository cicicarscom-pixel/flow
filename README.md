...
21. **Sistem Talimatı Kartına Standart Kalınlıkta Gök Mavisi Neon Çerçeve ve Belirgin Aura Gölgesi (UX/UI):**
    - **Neden:** Kartın tüm çerçevesinin standart kalınlıkta, sürekli parlayan gök mavisi renkli bir sınır çizgisine sahip olması ve arkasında belirgin bir gök mavisi aura gölgesi (shadow glow) oluşturulması istendi. Ancak, ScrollView padding'inin arayüz motoru tarafından tanınmaması nedeniyle kartın ekran sınırlarına kadar yapışarak yan çizgiler halinde görünmesi ve köşelerinin keskinleşmesi sorunu vardı. Ayrıca, kartın yarı saydam arka planından dolayı Android'in native `elevation` gölgesi içten sızarak gök mavisi sınır çizgisinin altında koyu gri ikinci bir çerçeve/gölge oluşturmaktaydı.
    - **Çözüm:** `BotYonetimiScreen.js` dosyasında şu düzenlemeler yapıldı:
        ...
        - **İç Gölgelenme Hatasının Engellenmesi (Solid Background):** `glowBorderCyanThick` stilinin arka planı yarı-saydam yerine opak solid koyu gri (`#1c1b1d`) yapıldı. Bu sayede Android işletim sisteminde `elevation: 10` nedeniyle oluşan sistem gölgesinin içeriden sızarak mavi çizginin altında ikinci bir koyu çerçeve oluşturması tamamen engellendi.
        - **Gök Mavisi Çerçeve ve Belirgin Aura Gölgesi:** Kenarları kaplayan `borderWidth: 1.5` kalınlığında solid `#00a2ff` (parlak gök mavisi) sınır çizgisi ve kartın arkasında `shadowColor: '#00a2ff'`, `shadowOpacity: 0.8`, `shadowRadius: 18` değerlerinde güçlü bir gök mavisi aura gölgesi sağlandı.
