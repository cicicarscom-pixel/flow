...
21. **Sistem Talimatı Kartına Conic Gradient RGB Işık Çerçevesi ve Aura Gölgesi (UX/UI):**
    - **Neden:** Kartın arkasında dönen gradyan katmanı bir kare olduğu için köşeleri dönme esnasında dışa taşmaktaydı. Ayrıca linear gradyanlar tek doğrultuda değiştiğinden tam bir conic gradient (çerçeveyi saran kesintisiz renk akışı) hissi vermiyordu. Kullanıcı, Google Stitch tasarımına benzer şekilde kartın yuvarlatılmış köşelerini takip eden parlayan kesintisiz bir RGB conic çerçeve ve onun arkasında aura şeklinde dönen bir RGB gölge (conic glow shadow) talep etti.
    - **Çözüm:** `BotYonetimiScreen.js` dosyasında şu düzenlemeler yapıldı:
        ...
        - **Dönen RGB Aura Gölgesi:** Kartın hemen arkasına, her yönden `30px` dışa taşan ve `blurRadius={Platform.OS === 'ios' ? 25 : 12}` ile yumuşatılmış bir `Animated.Image` (conic_glow) yerleştirildi. Dairesel yapı ve blur sayesinde, hiçbir köşe taşması olmadan kartın etrafında pürüzsüzce dönen bir aura gölgesi elde edildi.
        - **İç Dönen RGB Işık Çerçevesi (Border Beam):** Kartın `overflow: 'hidden'` olan 2.5px'lik kenarlık (padding) alanının içerisine, dönen `conic_glow` imajı yerleştirilerek çerçeve boyunca akan kesintisiz bir conic border beam oluşturuldu.
