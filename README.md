...
1. **Kritik Bulgu:** İnceleme sonucu, mobildeki "İleri Seviye Ayarlar" panelinde gösterilen "AI Karakter Talimatı (Prompt)" metninin (RULES'teki "polyglot" kültürel/dil adaptasyon kuralı dahil) client tarafında (OrchestrationEngine/PromptFactory/BlueprintFactory/GeminiRenderer) hesaplanan, salt-okunur bir önizleme olduğu ve gerçek müşteri botuna HİÇBİR ZAMAN ulaşmadığı doğrulandı — gerçek bot davranışı her zaman sunucu tarafında (ledger reposu, PersonaService + PersonaPromptBuilder) organization_ai_settings okunarak belirleniyordu.
2. **Kültürel/Dil Adaptasyonu Sunucuya (Ledger) Taşındı:** Bu platform uluslararası müşterilere hizmet verdiği için, kültürel/dil adaptasyonu artık configüre edilebilir bir ayar değil — ledger reposundaki `shared/ai/PromptBuilder.ts` dosyasının `SYSTEM_POLICY`'sine yeni, her zaman geçerli ve kapatılamaz bir kural (madde 1: "DİL VE KÜLTÜREL ADAPTASYON") eklendi. Müşteri hangi dilde yazarsa bot o dilde, o kültürün günlük ifade/espri anlayışına uygun şekilde cevap vermek zorunda — persona/karakter/rol seçiminden tamamen bağımsız.
3. **"İleri Seviye Ayarlar" Paneli ve Ölü Kod Temizliği (Mobil):** Artık işlevsiz/yanıltıcı hale gelen "İleri Seviye Ayarlar" paneli (ÖZEL KURALLARI AKTİFLEŞTİR anahtarı + salt-okunur prompt kutusu) BotYonetimiScreen.js'ten tamamen kaldırıldı. Bu paneli besleyen tüm client-side prompt hesaplama katmanı (`application/engines/OrchestrationEngine.ts`, `application/factories/PromptFactory.ts`+`BlueprintFactory.ts`, `application/renderers/BaseRenderer.ts`+`GeminiRenderer.ts`, `domain/types/EngineTypes.ts`, `domain/config/personas.ts`, `domain/config/rules.ts`) silindi. `usePersonaEngine`/`useSavePersona`/`SupabasePersonaRepository.savePersonaConfig` sadeleştirilerek `bot_settings.system_prompt/prompt_config/engine_mode` kolonlarına artık hiçbir şey yazılmıyor (web'in Phase 5 guardrail #2'siyle birebir aynı hizaya getirildi).
4. **Tek Gerçek Yapı:** Artık hem mobil hem web için tek bir gerçek akış var: UI seçimleri → `organization_ai_settings` → sunucuda `PersonaService` + `PersonaPromptBuilder` → gerçek prompt. Web tarafındaki zaten donmuş/işlevsiz "İleri Seviye Ayarlar" (AdvancedPersonaSettings.tsx) paneli de aynı sebeple flowweb reposundan kaldırıldı.
