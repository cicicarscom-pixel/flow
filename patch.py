...
file_path = 'c:/Users/roman/AI-Esnaf/src/modules/sosyal_medya/presentation/screens/BotYonetimiScreen.js'
with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()

# 1. Add state variable for accordion
if 'const [isAdvancedExpanded, setIsAdvancedExpanded] = useState(false);' not in content:
...
match = re.search(r'(return \(\s*<KeyboardAvoidingView.*?\);)', content, re.DOTALL)
if not match:
    print('Could not find return block')
    sys.exit(1)

old_return_block = match.group(1)
